  ...
  fluidRow(
    column(12, selectInput("filter_postcode", "Filtrer par code postal:", c("Tous", sort(unique(data$code_postal))))
      ...
      tabItem("dashboard_stats",
              ...
              fluidRow(
                plotlyOutput("pie_chart", width = "100%")  # Ajout du graphique à secteurs ici
              )
      )
    )
  )
...
server <- function(input, output) {
  ...
  output$boxplot_occupation <- renderPlot({
    p <- plot_ly(y = (data$available_bike_stands / data$bike_stands) * 100, type = "box") %>%
      layout(title = "Taux d'Occupation des Stations", yaxis = list(title = "Taux d'Occupation"))
    p
  })

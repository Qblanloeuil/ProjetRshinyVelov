...
# Calcul de l'Indice de Disponibilite des Stations de Vélos (IDSV)
...
server <- function(input, output) {
  ...
  # Crée une sortie "export_pie_chart_button" dans une application Shiny
  output$export_pie_chart_button <- downloadHandler(
    ...
    content = function(file) {
      # Créez le graphique à secteurs avec plotly
      # Créez un graphique à secteurs (camembert) en utilisant Plotly.
      # Les données sont spécifiées à partir d'un dataframe avec deux catégories : "Vélos disponibles" et "Nombre de supports vides".
      # Les valeurs correspondent aux sommes des vélos disponibles et des supports vides dans le dataframe 'data'.
      ...
      p <- plot_ly(data = data.frame(Category = c("Vélos disponibles", "Nombre de supports vides"),
        ...
        layout(title = "Taux de Disponibilité par Catégorie", showlegend = TRUE) %>%
        #  la mise en page du graphique à secteurs (camembert) pour afficher des informations spécifiques.
        layout(pie = list(textinfo = "label+percent"))
